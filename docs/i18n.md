...
**German ships with the skin** — `lang/de.conf` is a complete translation, kept complete by
a test, so new strings cannot ship untranslated.  It is **Beta** in 1.12: the mechanism is
tested and stable, the words await native-speaker review — a clumsy phrase is the worst a
Beta translation can do (a broken one falls back to English per string), and corrections are
