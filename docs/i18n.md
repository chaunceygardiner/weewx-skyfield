...
Dates split two ways.  Their *order and punctuation* are in the dictionary — the three
...
contributed by native speaker Gert Andersen, and `lang/nl.conf` and `lang/es.conf` are
complete Betas awaiting their native-speaker reviews — each kept complete by a test, so
new strings cannot ship untranslated.  Corrections are welcome as issues or pull
requests:
...
The complete reference dictionary, as shipped in 1.17 (`skins/Skyfield/lang/en.conf` in the
