...
moment the change is made -- not after the page has shipped and a reader
has found it.  It takes a second:

    tests/contrast.py '#ffffff' '#ff0000'
    tests/contrast.py '#1a0000' 'rgba(255,255,255,.55)' '#ff0000'
...
mid-luminance colors.  Dark ink on a pure red scores 5.03 -- comfortably
over AA -- and on a phone it cannot be read.  APCA, the model written to
...
The arithmetic below is shared, byte for byte, with the author's other
extensions, so every one of them measures a color the same way.  Change it
in all of them or in none.  tests/test_sky_page.py imports wcag(), apca()
and flatten() from this file, so the suite and the command line cannot
