...
**German, French and Dutch ship with the skin** — `lang/de.conf` and `lang/fr.conf` are
complete, native-speaker-reviewed translations and `lang/nl.conf` a complete Beta awaiting
its native-speaker review, each kept complete by a test, so new strings cannot ship
