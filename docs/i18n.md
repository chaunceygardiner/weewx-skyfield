...
**German, French, Danish, Dutch, Spanish, Italian, Norwegian and Swedish ship with the
skin** — `lang/de.conf` and `lang/fr.conf` are complete, native-speaker-reviewed
translations, `lang/da.conf` was contributed by native speaker Gert Andersen, and
`lang/nl.conf`, `lang/es.conf`, `lang/it.conf`, `lang/no.conf` (Norwegian Bokmål) and
`lang/sv.conf` are complete Betas awaiting their native-speaker reviews — each kept
complete by a test, so new strings cannot ship untranslated.  Corrections are welcome as
